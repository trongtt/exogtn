...
public class TestTransientTokenService extends AbstractTokenServiceTest<TransientTokenService>
{
   ...
   public void testExpire() throws Exception
   {
      ...
      for(String tokenId : service.getAllTokens())
      {
         assertTrue(service.getToken(tokenId).isExpired());
      }
      service.deleteToken(tokenId1);
      service.deleteToken(tokenId2);
   }
   ...
   public void testCleanExpireToken() throws Exception
   {
      ...
      do
      {
         flag = service.getToken(token1).isExpired() && service.getToken(token2).isExpired();
      }
      while (!flag);
