...
public class TestCookieService extends AbstractTokenServiceTest<CookieTokenService>
{
   ...
   {
      String tokenId = service.createToken(new Credentials("root", "gtn"));
      Token token = service.getToken(tokenId);
      
      Thread.sleep(2000); //for token expire
      assertTrue(token.isExpired());
      assertNull(service.validateToken(tokenId, false));
      
      Thread.sleep(3000); //for garbage thread clear token expired
   ...
   public void testCleanExpireToken() throws Exception
   {
      String token1 = service.createToken(new Credentials("root", "gtn"));
      String token2 = service.createToken(new Credentials("root1", "gtn1"));
      String token3 = service.createToken(new Credentials("root2", "gtn2"));
      
      assertEquals(service.size(), 3);
      ...
      assertEquals(service.size(), 0);
      assertEquals(service.getAllTokens().length, 0);
      assertNull(service.getToken(token1));
      assertNull(service.getToken(token2));
      assertNull(service.getToken(token3));
   }
}
