 ...
 * @version $Revision$
 */
public class GateInConsumerManagement extends HttpServlet
{
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
   {
      ...
      try
      {
         consumer = CookieConsumer.getConsumer("gateinConsumerManagement", getServletContext());
         OAuthAccessor accessor = CookieConsumer.getAccessor(request, response, consumer);
         OAuthMessage message =
            accessor.newRequestMessage(OAuthMessage.GET, "http://localhost:8080/exo-oauth-provider/rest/consumers",
               null);
